...
class MonthlyView extends React.Component {
  ...
  async changeMonth(e) {
    ...
    let data = await fetch(
      import.meta.env.VITE_SECONDARY_SERVER_URL + "/getTasksForMonth",
      {
        method: "POST",
        body: JSON.stringify({
          date: dateToString(date).replace(/ +/g, "-"),
    ...
    this.setState(
      { currentDate: date, selectedDate: selectedDate, tasksList: data },
      () => {
  ...
  generateDates() {
    ...
    //generating dates
    for (
      let dt = new Date(currentDate.getFullYear(), currentDate.getMonth(), 1);
      dt <= new Date(currentDate.getFullYear(), currentDate.getMonth() + 1, 0);
      dt.setDate(dt.getDate() + 1)
    ) {
      button = document.createElement("button");
      if (dt.toDateString() === this.state.currentDate.toDateString())
  ...
  selectDate(e) {
    let target = e
      ? e.currentTarget
      : document.querySelector("#datesContainer").childNodes[
    ...
    if (prevTarget) {
      if (
        new Date(
          `${prevTarget.querySelector("i").innerText} ${
            document.querySelector("#month_name").innerText
          }`
  ...
  displayTasks() {
    ...
    for (let j = 0; j < tasksList.length; j++) {
      ...
      if (
        i.getDate() !== this.state.selectedDate ||
        i.getMonth() !== this.state.currentDate.getMonth() ||
        i.getFullYear() !== this.state.currentDate.getFullYear()
      )
        continue;
  ...
  completeTask() {
    ...
    fetch(import.meta.env.VITE_SECONDARY_SERVER_URL + "/completeTask", {
      ...
      body: JSON.stringify({
        id: this.currentTask,
        broker_id: this.props.getItem("id"),
        outcome: outcome,
      }),
      headers: {
        "Content-type": "application/json; charset=UTF-8",
  ...
  toggleCompletedTaskMenu(e) {
    let completedTaskMenu =
      this.state.completedTaskMenu === undefined
        ? "none"
        : this.state.completedTaskMenu;
      ...
      <section id="monthly_view">
        ...
        <div className="calendar">
          <div className="month">
            ...
            <div id="month_name">
              {months[this.state.currentDate.getMonth()].slice(0, 3)}{" "}
        ...
        <div id="main">
          ...
          <SnoozeMenu
            display={this.state.snoozeTaskMenu}
            toggleSnoozeTaskMenu={this.toggleSnoozeTaskMenu}
