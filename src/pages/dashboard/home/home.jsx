...
class HomeComp extends React.Component {
  ...
  generateDates() {
    ...
    // this.generateTasks([])
    for (
      let dt = new Date(today.getFullYear(), today.getMonth(), 1);
      ...
      if (dateIndex < this.date.length) {
        if (
          dt.toDateString() ===
          new Date(this.date[dateIndex].date).toDateString()
        ) {
        ...
        if (dt.toDateString() === today.toDateString()) {
          ...
          this.setState({ lastSelectedDate: dt.getDate() }, () => {
            document
              ...
              [this.state.lastSelectedDate - 1].scrollIntoView({
                behavior: "smooth",
                inline: "center",
              });
          });
  ...
  getTasksPerDate(e) {
    ...
    if (
      lastSelectedDateDiv.querySelector("#date").innerText ===
      new Date().getDate()
    ) {
      lastSelectedDateDiv.querySelector("#date").style.backgroundColor =
        "#a5b3cd";
    ...
    let nextElementIndex =
      Array.prototype.indexOf.call(dateDivList, e.currentTarget) + 1;
  ...
  componentDidMount() {
    fetch(import.meta.env.VITE_SECONDARY_SERVER_URL + "/getTasksForMonth", {
      method: "POST",
      body: JSON.stringify({
        date: dateToString(this.state.today).replace(/ +/g, "-"),
        broker_id: localStorage.getItem("id"),
      }),
      headers: {
        "Content-type": "application/json; charset=UTF-8",
  ...
  toggleSnoozeTaskMenu(e) {
    ...
    if (display === "none") {
      ...
      let dateDiv =
        document.querySelectorAll(".date")[this.state.lastSelectedDate - 1];
      if (dateDiv)
        this.generateTasks(
          dateDiv.name === -1 ? [] : this.date[dateDiv.name].tasks
        );
      ...
      <div id="homeMain">
        ...
        <div id="reminderDiv">
          <div id="calendar">
            <div id="months">
              <p>
                {months[this.state.today.getMonth()].slice(0, 3)}{" "}
