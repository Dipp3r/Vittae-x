...
class CustomerView extends React.Component {
  ...
  addTask() {
    ...
    obj.type = 0;
    obj.id = Number.parseInt(
      `${this.state.customer.id}${this.state.customer.tasks.length}`
    );
  ...
  emptyAddTaskMenu() {
    ...
    menu.querySelector("#addTaskDate").value = dateToString(
      new Date(),
      2
    ).replace(/ /g, "-");
  ...
  toggleCompletedTaskMenu(e) {
    ...
    if (completedTaskMenu === "flex") {
      ...
      menu.querySelector("#date").value = dateToString(
        new Date(taskObj.date),
        2
      ).replace(/ /g, "-");
      menu.querySelector("#time").value =
        new Date(taskObj.date).getHours().toString().padStart(2, "0") +
        ":" +
        new Date(taskObj.date).getMinutes().toString().padStart(2, "0");
  ...
  changeSection(e) {
    ...
    let lastTarget =
      document.querySelectorAll(".statusButton")[
        Number.parseInt(this.state.section) - 1
      ];
  ...
  generateTasks(tasksList) {
    ...
    for (let task of tasksList) {
      ...
      time.innerText =
        new Date(taskDate).getHours().toString().padStart(2, "0") +
        ":" +
        new Date(taskDate).getMinutes().toString().padStart(2, "0");
  ...
  toggleAddNotesPage() {
    ...
    if (notesPg === "none") {
      ...
      if (noteTitle.value !== "" && noteBody.value !== "") {
        ...
        let data = {
          id: Number.parseInt(
            `${this.state.customer.id}${customer.notes.length}`
          ),
  ...
  updateClientInfo() {
    ...
    fetch(
      import.meta.env.VITE_BASE_SERVER_URL +
        `/customer-onboarding/${this.state.customer.id}/`,
      {
        method: "PATCH",
        body: JSON.stringify(obj),
        headers: {
  ...
  componentDidMount() {
    ...
    try {
      fetch(
        import.meta.env.VITE_BASE_SERVER_URL + `/customer-detail/${obj.id}/`,
        {
          method: "GET",
          headers: {
            Authorization: `Passcode ${localStorage.getItem("token")}`,
        ...
        .then(async (data) => {
          ...
          //getting notes tasks
          data.notes = await fetch(
            import.meta.env.VITE_SECONDARY_SERVER_URL + "/getNotesList",
            {
              method: "POST",
              body: JSON.stringify({
                customer_id: obj.id,
          ...
          //getting taks list
          data.tasks = await fetch(
            import.meta.env.VITE_SECONDARY_SERVER_URL + "/getTasksList",
            {
              method: "POST",
              body: JSON.stringify({
                customer_id: obj.id,
  ...
  render() {
    ...
    return (
      <section id="Client">
        ...
        <div
          id="personalInfo"
          style={{ display: this.state.editCustomerDetail }}
        >
          ...
          <div className="inputField">
            ...
            <input
              ...
              className="field inputField"
              name="occupation"
              defaultValue={this.state.customer.occupation}
            />
            ...
            <div id="dobField">
              <input
                ...
                name="date_of_birth"
                value={dateToString(
                  new Date(this.state.customer.date_of_birth),
                  2
                ).replace(/ /g, "-")}
                onChange={this.changeInDateValue}
              />
            </div>
          </div>
        ...
        <div id="main">
            ...
            <div id="portion1">
              <div id="portion1Left">
                ...
                <div id="deets">
                  <div id="edit">
                    <p id="name">
                      {this.state.customer.first_name
                        ? this.state.customer.first_name.length <= 10
                          ? this.state.customer.first_name
                          : `${this.state.customer.first_name.slice(0, 7)}...`
                        : ""}
                    </p>
                  ...
                  <p id="designation">
                    {this.state.customer.occupation.length < 15
                      ? this.state.customer.occupation
                      : this.state.customer.occupation.splice(12)}
                  </p>
                  <div id="statusDiv">
                    <p
                      id="statusDot"
                      style={{
                        color: this.state.customer.statusBackgroundColor,
                      }}
                    >
                      .
                    </p>
                    ...
                    >
                      {this.state.customer.statusText}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div id="portion1Right">
                <div id="dateBox">
                  <p id="dateTxt">
                    Added{" "}
                    {new Date(this.state.customer.created_at).toDateString()}
                  </p>
                </div>
          ...
          <div id="infoContent">
              ...
              <div className="grid-container">
                ...
                <div className="grid-item info">
                  {new Date().getFullYear() -
                    new Date(this.state.customer.date_of_birth).getFullYear()}
                </div>
                <div className="grid-item">Gender</div>
                <div className="grid-item info">
                  {this.state.customer.gender_name}
                </div>
            ...
            <div className="infoDiv">
              <div className="grid-container">
                ...
                <div className="grid-item emptyInfo">
                  {this.state.customer.pan_number ? "##########" : "---"}
                </div>
          ...
          <div id="kycStatus">
            {this.state.KYCLog.length > 0 && (
              <KYCsteps items={this.state.KYCLog} />
            )}
          </div>
          ...
          <div
            id="addTaskScreen"
            style={{
              display: this.state.addTaskMenu,
              zIndex: 2,
              position: "absolute ",
            }}
          >
                ...
                <div id="fieldDiv">
                  <div className="field">
                    ...
                    <input
                      type="date"
                      id="addTaskDate"
                      defaultValue={dateToString(new Date(), 2).replace(
                        / /g,
                        "-"
                      )}
                    />
          ...
          <div
            id="completedTaskScreen"
            style={{
              display: this.state.completedTaskMenu,
              zIndex: 2,
              position: "absolute ",
            }}
          >
