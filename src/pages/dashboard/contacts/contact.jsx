...
class ContactsComp extends React.Component {
  ...
  openCustomerView(e) {
    ...
    for (let i of this.customerList) {
      if (i.id === target) {
        // eslint-disable-next-line react/prop-types
        this.props.setItem(
          { contactCompState: this.state, currentCustomerView: i },
          () => {
            // eslint-disable-next-line react/prop-types
  ...
  setFilterPropStatus(e) {
    ...
    this.lastSelectedFilterButton.style.borderBottomColor =
      "rgba(109, 117, 147, 1)";
  ...
  filterAndSort(customerList) {
    ...
    switch (filterProps.sort) {
      default:
      case "dateDesc":
        newCustomerList = newCustomerList.sort((a, b) => {
          return (
  ...
  async submit() {
    ...
    let data = await fetch(
      import.meta.env.VITE_BASE_SERVER_URL + "/customer-onboarding/",
      {
        method: "POST",
    ...
    fetch(import.meta.env.VITE_SECONDARY_SERVER_URL + "/addCustomer", {
      method: "POST",
    ...
    fetch(
      import.meta.env.VITE_BASE_SERVER_URL +
        "/customer-list/?page=1&page_size=10",
      {
        method: "GET",
        headers: {
          Authorization: `Passcode ${localStorage.getItem("token")}`,
  ...
  scrollingList(e) {
    ...
    if (
      scrollValue === maxScrollableHeight &&
      current_page < this.state.num_pages
    ) {
      current_page += 1;
  ...
  async fetchCustomersList() {
    ...
    let dataUrl =
      import.meta.env.VITE_BASE_SERVER_URL +
      `/customer-list/?page=${page}&page_size=10`;
  ...
  componentDidMount() {
    ...
    this.setState(obj, () => {
      ...
      for (let i of document.querySelector("#filterLabels").children) {
        if (
          new RegExp(i.getAttribute("value")).test(
            this.state.filterProps.tag.join(" ")
          )
        ) {
      ...
      this.lastSelectedFilterButton =
        document.body.querySelector("#statusButton").children[0];
  ...
  render() {
    return (
      <div id="contactMain">
        <div id="statusBar">
          <div id="statusButton">
            <button
              ...
              value="0"
              onClick={this.setFilterPropStatus}
            >
              ALL
            </button>
            <button
        ...
        <div id="labelBar">
          <div id="labels">
            <button className="label" onClick={this.toggleFilterSort} value="2">
              Name
            </button>
            ...
            <button className="label row">
              ...
              <img
                ...
                alt="info_icon"
                onMouseEnter={() => {
                  document.querySelector("#infoCard").style.display = "flex";
                }}
                onMouseLeave={() => {
            ...
            <button className="label" onClick={this.toggleFilterSort} value="1">
              Added Date
            </button>
          </div>
        </div>
        ...
        <div
          className="scrolling-wrapperY"
          id="cardsList"
          onScroll={this.scrollingList}
        >
          {/* list of customers HERE  */}
          {this.state.customerCompList.map((element) => {
            return element;
          })}
          <div id="infoCard">
            ...
            <div className="infoRow">
              <div className="status status4"></div>
              <p>KYC done</p>
            </div>
          </div>
        </div>
        ...
        <div
          id="addCustomer"
          style={{ display: `${this.state.addClientMenu}` }}
        >
          <div id="addBox">
        ...
        <div id="filter" style={{ display: `${this.state.filterMenu}` }}>
          <div id="filterBox">
            ...
            <div id="filterLabels">
              <button
                className="fLabel"
                onClick={this.setFilterPropsTag}
                value="tag1"
              >
                tag1
              </button>
        ...
        <button id="add" onClick={this.toggleAddClientMenu}>
          {" "}
